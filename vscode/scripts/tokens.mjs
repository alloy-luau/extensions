...
const files = { "source.aly": "aly", "source.alx": "alx", "source.daly": "daly" };
const registry = new vsctm.Registry({
	...
	loadGrammar: async (scope) => {
		...
		const text = fs.readFileSync(path.join(root, "syntaxes", `${file}.tmLanguage.json`), "utf8");
...
const scope = file.endsWith(".alx") ? "source.alx" : file.endsWith(".d.aly") ? "source.daly" : "source.aly";
...
lines.forEach((line, i) => {
	...
	const parts = result.tokens.map((t) => {
		const scopes = t.scopes.slice(1).map((s) => s.replace(/\.(aly|alx|daly)$/, ""));
